...
export const DataEntry: React.FC<DataEntryProps> = ({ 
  ...
  onVideoUpload, onRemoveVideo, onAudioUpload, onRemoveAudio, onAddToBailHistory, onRemoveFromBailHistory
}) => {
      ...
      <div className="space-y-6 pb-20">
        <section className="bg-white p-4 border border-gray-100 rounded-lg shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="text-[10px] font-bold text-gray-400 block mb-1">पिता का नाम (Father's Name)</label>
              <textarea
                name="bail_father_name"
                value={state.fields.bail_father_name || ''}
                onChange={(e) => onFieldChange(e.target.name, e.target.value)}
                className="w-full border border-gray-200 rounded p-2 text-xs focus:ring-1 focus:ring-blue-500 focus:outline-none"
              />
            </div>
            ...
            <div>
              <label className="text-[10px] font-bold text-gray-400 block mb-1">स्थायी पता (Permanent Address)</label>
              <textarea
                name="bail_address"
                value={state.fields.bail_address || ''}
                onChange={(e) => onFieldChange(e.target.name, e.target.value)}
            ...
            {/* History Log Management in Sidebar */}
            {state.bailHistory.length > 0 && (
              <div className="mt-6 border-t pt-4">
                <h4 className="text-[10px] font-bold text-gray-400 uppercase mb-2">History Log Management</h4>
                <div className="space-y-2 max-h-[200px] overflow-y-auto pr-1">
                  {state.bailHistory.map((entry, idx) => (
                    <div key={idx} className="bg-gray-50 border border-gray-200 p-2 rounded flex justify-between items-start gap-2 shadow-sm">
                      <div className="text-[9px] flex-1 min-w-0">
                        <p className="font-bold text-indigo-700">{entry.date}</p>
                        <p className="text-gray-600 truncate">{entry.name}</p>
                      </div>
                      <button 
